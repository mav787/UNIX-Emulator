...
public class MyFileSystem implements FileSystem {
        ...
        if(blockNum > 9) { 
        	...
        	if (blockNum < 10 + IndirectBlock.COUNT + IndirectBlock.COUNT*IndirectBlock.COUNT +IndirectBlock.COUNT*IndirectBlock.COUNT*IndirectBlock.COUNT){
        		...
        		if(blockNum < 10 + IndirectBlock.COUNT){
        			...
        			return singleIndirect(blockNum, blockOff, inode, mode);
        		}
        		else if(blockNum < 10 + IndirectBlock.COUNT + IndirectBlock.COUNT*IndirectBlock.COUNT){
        			
        			// double indirect case
        			return doubleIndirect(blockNum, blockOff, inode, mode);
        		}
        		else{
        			
        		    // triple indirect case
        			return tripleIndirect(blockNum, blockOff, inode, mode);
        		}
        	}
	        else {
		       // not legal
    ...
    //
    private DirectBlock direction(IndirectBlock iBlock, int index, int addr, int blockOff, MODE mode){
    	boolean fresh = iBlock.ptr[index] == 0;
        if(fresh){
	        if(mode == MODE.r){
	            return DirectBlock.hole;
	        }
	        else if((iBlock.ptr[index] = freeMap.find()) == 0){
	        	return null;
	        }
        }
        disk.write(addr, iBlock);
        return new DirectBlock(disk, iBlock.ptr[index], blockOff, fresh);
    }
    
    
    
    // get indirect block from indirect block, return false if disk is full
    private boolean getIndirectBlock(IndirectBlock iBlock, int index, int addr, IndirectBlock iBlockObj){
    	if(iBlock.ptr[index] == 0) {
    		
    		if((iBlock.ptr[index] = freeMap.find()) == 0) {
    			return false;
    		}
    		// store this iBlock, ptr[] modified
    		disk.write(addr, iBlock);
    		// update the free map
    		freeMap.save();
    	}
    	else {
    		disk.read(iBlock.ptr[index], iBlockObj);
    	}
    	return true;
    }
    
    // get indirect block from inode, return false if disk is full
    private boolean getIndirectBlock(Inode inode, int index, IndirectBlock iBlockObj){
    	
    	if(inode.ptr[index] == 0) {
    		if((inode.ptr[index] = freeMap.find()) == 0) {
    			return false;
    		}
    		// inode.ptr[] get reassigned by freeMap.find()....
    		disk.write(inode.ptr[index], iBlockObj);
    		// update the free map
    		freeMap.save();
    	}
    	else {
    		disk.read(inode.ptr[index], iBlockObj);
    	}
    	return true;
    }
    ...
    // single indirect
    private DirectBlock singleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock firstIndirect = new IndirectBlock();
    	int index = blockNum - 10;    
    	
    	boolean IndirectblockNotExist = !getIndirectBlock(inode, 10, firstIndirect);
    	if(IndirectblockNotExist) {
    		return null;
    	}
    	return direction(firstIndirect, index, inode.ptr[10], blockOff, mode);
    }
    
    
    // double indirect
    private DirectBlock doubleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock firstIndirect = new IndirectBlock();
    	IndirectBlock secondIndirect = new IndirectBlock();

    	int firstIndex = (blockNum - 10 - IndirectBlock.COUNT) / IndirectBlock.COUNT;
    	int secondIndex = (blockNum - 10 - IndirectBlock.COUNT) % IndirectBlock.COUNT;
    	
    	boolean Indirect1NotExist =  !getIndirectBlock(inode, 11, firstIndirect); 
    	boolean Indirect2NotExist =	 !getIndirectBlock(firstIndirect, firstIndex, inode.ptr[11], secondIndirect);
    	if(Indirect1NotExist || Indirect2NotExist){
    		return null;
    	}
    	return direction(secondIndirect, secondIndex, firstIndirect.ptr[firstIndex], blockOff, mode);
    }
    
    
    // triple indirect
    private DirectBlock tripleIndirect(int blockNum, int blockOff, Inode inode, MODE mode){
    	IndirectBlock firstIndirect = new IndirectBlock();
    	IndirectBlock secondIndirect = new IndirectBlock();
    	IndirectBlock thirdIndirect = new IndirectBlock();
    	        
        int firstIndex = (blockNum - 10 - IndirectBlock.COUNT - IndirectBlock.COUNT*IndirectBlock.COUNT) / (IndirectBlock.COUNT * IndirectBlock.COUNT);
       	int secondIndex = ((blockNum - 10 - IndirectBlock.COUNT - IndirectBlock.COUNT*IndirectBlock.COUNT) % (IndirectBlock.COUNT*IndirectBlock.COUNT))/IndirectBlock.COUNT;
       	int thirdIndex =  (blockNum - 10 - IndirectBlock.COUNT - IndirectBlock.COUNT*IndirectBlock.COUNT) % IndirectBlock.COUNT;
       	
       	boolean Indirect1NotExist = !getIndirectBlock(inode, 12, firstIndirect);
       	boolean Indirect2NotExist = !getIndirectBlock(firstIndirect, firstIndex, inode.ptr[12], secondIndirect);
       	boolean Indirect3NotExist = !getIndirectBlock(secondIndirect, secondIndex, firstIndirect.ptr[firstIndex],thirdIndirect);
       	if(Indirect1NotExist || Indirect2NotExist || Indirect3NotExist) {
       		return null;
       	}
       	return direction(thirdIndirect, thirdIndex, secondIndirect.ptr[secondIndex], blockOff, mode);
    }
